...
const CONFIG = {
  ...
  tiktok    : "https://www.tiktok.com/@genstorege",
  whatsapp  : "500700362",
  phone     : "",
...
async function renderSvcAdminList() {
  ...
  const alertCnt = orders.filter(o => o.status === "new" || o.status === "confirmed").length;
  if (badge) { badge.textContent = alertCnt || ""; badge.classList.toggle("hidden", !alertCnt); }
...
async function confirmSvcOrder(id) {
  try {
    await db.collection("service_orders").doc(id).update({ status: "confirmed" });
    toast("შეკვეთა დადასტურდა ✓");
    renderProfile();
  } catch { toast("შეცდომა"); }
}

async function rejectSvcOrder(id) {
  try {
    await db.collection("service_orders").doc(id).update({ status: "rejected" });
    toast("შეკვეთა უარყოფილია");
    renderProfile();
  } catch { toast("შეცდომა"); }
}
...
function renderProductModal(id) {
  ...
  const ctaTt = CONFIG.tiktok
    ? `<a class="btn btn-cta btn-cta-tt" href="${CONFIG.tiktok}" target="_blank" rel="noopener">
    <svg viewBox="0 0 24 24" width="17" height="17" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.69a8.18 8.18 0 004.78 1.52V6.75a4.85 4.85 0 01-1.01-.06z"/></svg>
    TikTok
  </a>` : "";
  ...
  <div class="modal">
    <div class="modal-top">
      ...
      <div class="modal-info">
        ...
        ${p.sold ? `<div class="sold-badge-modal">გაყიდულია</div>` : `<div class="cta">${ctaFb}${ctaIg}${ctaTt}${ctaWa}${ctaPhone}</div>`}
      </div>
    </div>
  </div>
</div>`;
  ...
  if (ordBox) {
    ...
    try {
      ...
      if (!orders.length) {
        ...
        const waAdminRaw = (CONFIG.whatsapp || "").replace(/\D/g, "");
        const waAdmin = waAdminRaw.startsWith("995") ? waAdminRaw : "995" + waAdminRaw;
        ...
        ordBox.innerHTML = orders.map(o => {
          ...
          const photos = o.photos && o.photos.length
            ? `<div class="prof-order-photos">${o.photos.map((ph, i) =>
                `<img src="${ph}" class="prof-order-photo" title="${i===0?"ლეპტოპი":"სერიული ნომერი"}" onclick="openSvcPhotoModal(this.src)">`
              ).join("")}</div>`
            : "";

          const total = Number(o.price || 0) + (o.install ? Number(o.installPrice || 0) : 0);
          const priceBox = o.price
            ? `<div class="prof-order-price-box">
                <div class="prof-order-price-row">
                  <span>დეტალი</span><span><b>${o.price}₾</b></span>
                </div>
                ${o.install && o.installPrice ? `<div class="prof-order-price-row">
                  <span>მონტაჟი</span><span><b>${o.installPrice}₾</b></span>
                </div>` : ""}
                <div class="prof-order-price-row">
                  <span class="prof-order-total-lbl">სულ გადასახდელი</span>
                  <span class="prof-order-total-val">${total}₾</span>
                </div>
               </div>`
            : "";

          const actionBox = o.status === "priced"
            ? `<div class="prof-order-action-box">
                <div class="prof-order-action-title">
                  ...
                  ფასი მოვიდა — დაადასტურეთ ან უარყავით
                </div>
                <div class="prof-order-confirm">
                  <button class="btn btn-theme btn-sm" onclick="confirmSvcOrder('${o.id}')">✓ &nbsp;დადასტურება</button>
                  <button class="btn btn-danger btn-sm" onclick="rejectSvcOrder('${o.id}')">✗ &nbsp;უარყოფა</button>
                </div>
               </div>`
            : "";
          ...
          return `<div class="prof-order">
            <div class="prof-order-body">
              ...
              <div class="prof-order-tags">
                ...
                <span class="svc-status ${o.status} prof-status-pill">${esc(label)}</span>
              </div>
              <div class="prof-order-meta">
                ...
                ${date}
                ${o.contact ? `&nbsp;·&nbsp;<svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 11a19.79 19.79 0 01-3.07-8.67A2 2 0 012 .18h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L6.09 7.91a16 16 0 006 6l1.19-1.19a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 14.92z"/></svg>&nbsp;${esc(o.contact)}` : ""}
              </div>
              ${photos}
              ${priceBox}
            </div>
            ${actionBox}
            <div class="prof-order-footer">
              <span class="prof-order-date-lbl">${label}</span>
              <button class="prof-order-wa" onclick="window.open('https://wa.me/${waAdmin}','_blank')">${waSvg} WA-ით დაწერე</button>
            </div>
          </div>`;
        }).join("");
      }
